  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      try {
        ...
        const articleWithId = {
          ...article,
          _id: `${Date.now().toString()}-${(__saveIdCounter++).toString()}-${Math.random()
            .toString(36)
            .slice(2, 12)}`,
        ...
        // If this article has a URL, deduplicate by URL. If it has no URL, treat as a unique save.
        const existing = article?.url
          ? existingSavedArticles.find((a) => a.url === article.url)
          : undefined;
        if (existing) {
